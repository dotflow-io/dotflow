...
class TestTimeDecorator(unittest.TestCase):
    def test_duration_is_set_after_execution(self):
        @time
        def run():
            return Task(task_id="01ARZ3NDEKTSV4RRFFQ69G5FAV", step=action_step, callback=simple_callback)
    ...
    def test_return_value_is_preserved(self):
        expected_task = Task(
            task_id="01ARZ3NDEKTSV4RRFFQ69G5FAV", step=action_step, callback=simple_callback
        )
